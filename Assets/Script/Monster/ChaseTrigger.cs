...
public class ChaseTrigger : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("ChaseStart1"))
        {
            ...
            GuideTextManager.Instance.ShowMessage("누군가 따라오고 있다. 여자화장실로 들어가 숨어보자.");
        }
        else if (other.CompareTag("ChaseEnd1") && monster1.isChasing)
        {
            monster1.StopChase();
        ...
        else if (other.CompareTag("ChaseStart2"))
        {
            ...
            GuideTextManager.Instance.ShowMessage("누군가 따라오고 있다.. 어디론가 숨어야 할 것 같다.");

      
            if (chaseEndTrigger2 != null)
                chaseEndTrigger2.SetActive(true);
        }
        else if (other.CompareTag("ChaseEnd2") && monster2.isChasing)
        {
            monster2.StopChase();
            GuideTextManager.Instance.ShowMessage("추격이 멈춘 것 같다.");
        }
